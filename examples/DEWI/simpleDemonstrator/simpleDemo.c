...
int stringAddrtoInt_master_availability(linkaddr_t *tmp, char * data){
	...
	printf("tmp2 : %d:%d  node : %d:%d\n", tmp2.u8[0],tmp2.u8[1],linkaddr_node_addr.u8[0],linkaddr_node_addr.u8[1]);
	linkaddr_copy(tmp, &tmp2);
	if(linkaddr_node_addr.u8[0]==tmp2.u8[0] &&linkaddr_node_addr.u8[1]==tmp2.u8[1] ){
		return 1;
	}
	return 0;
...
//	  leds_toggle(LEDS_ALL);
	...
	if(strcmp(mssg, "M available")==0 && !master_addr1.u8[1]&&!master_addr1.u8[0]){
		printf("message %s received from %d:%d ..........  : %d : %d :\n",mssg, from->u8[0], from->u8[1], master_addr1.u8[0], master_addr1.u8[1]);
	   	...
	   	if(!runicast_is_transmitting(&data_runicast)) {  //save master address then send to the master node, self-address by unicast
    		...
    		printf("%u.%u: sending runicast to address %u.%u\n",linkaddr_node_addr.u8[0], 
    															linkaddr_node_addr.u8[1], master_addr1.u8[0], master_addr1.u8[1]);
		...
		printf("%s \r\n", mssg);
		if(linkaddr_node_addr.u8[0]==atoi(strncpy(temporal, strstr(mssg, "(")+1, 3))  //2  for erase the master node address if this slave node are disconnected
		 && linkaddr_node_addr.u8[0]==atoi(strncpy(temporal, strstr(mssg, ".")+1, 3))){  //2  if it doesn't work, erase all code with number 2 as comment.
			master_addr1.u8[0] = 0;															//2
			master_addr1.u8[1] = 0;														//2
		}
	}else if (strstr(mssg, "from")!=NULL && strstr(mssg, "tp")!=NULL && strstr(mssg, "bat")!=NULL ){
		...
		printf("node(%d:%d) Temperature = '%dC' \r\n",tmp->u8[0], tmp->u8[1], temperature_data);
		sendBatteryStatusByserialP(battery_data, tmp->u8[0], tmp->u8[1]);
	}else if(strcmp(mssg, "M available")!=0 && strstr(mssg, "MasterSlave_addr")== NULL&&strstr(mssg, "from")==NULL && strstr(mssg, "tp")==NULL && strstr(mssg, "bat")==NULL && linkaddr_cmp(&master_addr1,from)){
...
/*---------------------------------------------------------------------------*/
static const struct broadcast_callbacks colour_rx = { colorRecvd }, brightness_rx = {brightnessRecvd};
...
{ 
	PROCESS_BEGIN()
		...
		runicast_open(&data_runicast, ADDRMasterSlave_CHANNEL, &runicast_callbacks);
...
PROCESS_BEGIN();
	...
	while (1)
	{
		...
		else if(ev == serial_line_event_message) // if there are data sent in the serial port 
		{
			...
			if (strcmp(ch_data, "serial")==0)   // if serial port data is a 'serial' signal , it mean that this node will be enable to have new slaves nodes during 15 seconds 
			{
				...
				if (stringAddrtoInt_master_availability(&tmp1, ch_data))   // if it is itself 
				{
					...
					if(!runicast_is_transmitting(&data_runicast)){  // if it is another node
						packetbuf_copyfrom("Be available\0", sizeof("Be available\0"));
						runicast_send(&data_runicast, &tmp1, MAX_RETRANSMISSIONS);
			    	}		
			    	data_runicast.is_tx = 0;
				}
				...
				{	
					temperature = cc2538_temp_sensor.value(CC2538_SENSORS_VALUE_TYPE_CONVERTED)/1000;
					...
					sprintf(temp_battery, "from (%d:%d) tp:%dC bat:%d mV ",linkaddr_node_addr.u8[0], linkaddr_node_addr.u8[1], temperature, battery);  				
    		...
    		if(master_addr1.u8[0] > 0 && master_addr1.u8[1] > 0 && !linkaddr_cmp(&master_addr1,&linkaddr_node_addr)){
